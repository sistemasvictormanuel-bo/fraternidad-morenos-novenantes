...
$("#save").on("click", function() {
    // 1. Obtener el ID del fraterno del nuevo input
    const fraternoIdElement = document.getElementById('fraternoIdInput');
    const fraternoId = fraternoIdElement ? fraternoIdElement.value.trim() : null;

    if (!fraternoId || isNaN(parseInt(fraternoId))) {
        alert("Por favor, ingrese un ID de fraterno válido antes de guardar.");
        return;
    }

    // 2. Obtener la imagen Base64 del Local Storage (asumiendo formato PNG)
    // El PNG en Base64 está en localStorage.getItem("imageSrc")
    const imageBase64 = localStorage.getItem("imageSrc"); 

    if (!imageBase64 || imageBase64 === "") {
        alert("Error: No hay una huella PNG capturada para guardar.");
        return;
    }

    // 3. Enviar la huella al Agente Java
    showMessage("Guardando Feature Set, por favor espere...");
    enrolarHuella(fraternoId, imageBase64); // <<<< ¡AQUÍ ESTÁ EL CAMBIO!

    // *********
    // IMPORTANTE: El código original que manejaba la galería local
    // de imágenes (vDiv.children.length < 5, etc.) ha sido ELIMINADO.
    // Si aún lo necesitas, puedes incorporarlo aquí.
    // *********
});

$("#verifyButton").on("click", function() {
    // 1. Obtener la imagen Base64 del Local Storage
    const imageBase64 = localStorage.getItem("imageSrc"); 

    if (!imageBase64 || imageBase64 === "") {
        alert("Error: Primero, capture una huella digital.");
        return;
    }

    // 2. Ejecutar la función de verificación
    showMessage("Iniciando Verificación (Match 1:N), por favor espere...");
    
    // Esta función llama a http://localhost:8080/verify
    verificarHuella(imageBase64); 
});
...
/**
 * Envía el ID del fraterno y la imagen Base64 al servidor Node.js/Express.
 * @param {string} fraternoId - El ID del fraterno.
 * @param {string} imageDataUrl - La imagen Base64 con el prefijo "data:image/png;base64,...".
 */
// =========================================================================
//  NUEVAS FUNCIONES: CONEXIÓN AL AGENTE JAVA (SPARK EN PUERTO 8080)
// =========================================================================
var AGENT_BASE_URL = 'http://localhost:8080';

/**
 * 💾 FUNCIÓN PARA ENROLAR (Guardar Huella)
 * Flujo: Web Client (PNG) -> Agente Java (crea Feature Set) -> API Remota (guarda Feature Set)
 */async function enrolarHuella(fraternoId, imageBase64) {
    // 🛑 MODIFICACIÓN CLAVE: USAR .replace() para una limpieza más segura.
    // Elimina el prefijo 'data:image/png;base64,' o 'data:image/jpeg;base64,' si está presente.
    const cleanBase64 = imageBase64.replace(/^data:image\/(png|jpeg);base64,/, "");

    console.log("Iniciando enrolamiento en el Agente Java...");

    const payload = {
        fraterno_id: fraternoId.toString(),
        image_base64: cleanBase64 // <--- Usamos la cadena LIMPIA
    };

    try {
        const response = await fetch(`${AGENT_BASE_URL}/enroll`, { 
            method: 'POST',
        ...
        if (response.ok && data.success) {
            alert(`✅ ENROLAMIENTO EXITOSO: Feature Set guardado.`);
            showMessage(`¡Éxito! Huella guardada. ID DB: ${data.id}`);
            onClear();
        } else {
            alert(`🛑 ERROR EN EL AGENTE/API: ${data.message || 'Error desconocido'}`);
            showMessage(`Error al guardar huella: ${data.message}`);
        }
    } catch (error) {
        console.error("Error de conexión con el Agente Java:", error);
        alert("❌ Error: No se pudo conectar al Agente Local de Java (http://localhost:8080). Asegúrate de que esté corriendo.");
        showMessage('Error: No se pudo conectar al Agente Local de Java.');
    }
}

/**
 * 🔎 FUNCIÓN PARA VERIFICAR (Match 1:N)
 * Flujo: Web Client (PNG) -> Agente Java (compara 1:N contra BD remota)
 *//**
 * 🔎 FUNCIÓN PARA VERIFICAR (Match 1:N)
 * Flujo: Web Client (PNG) -> Agente Java (compara 1:N contra BD remota)
 */
async function verificarHuella(imageBase64) {
    // 🛑 MODIFICACIÓN CLAVE: USAR .replace() para una limpieza más segura.
    const cleanBase64 = imageBase64.replace(/^data:image\/(png|jpeg);base64,/, ""); 

    console.log("Iniciando verificación en el Agente Java...");
    ...
    try {
        const response = await fetch(`${AGENT_BASE_URL}/verify`, { 
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
        });
        ...
        if (data.success) {
            alert(`✅ MATCH ENCONTRADO: Fraterno ID ${data.fraterno_id}`);
            showMessage(`Coincidencia encontrada para ID: ${data.fraterno_id}`);
        } else {
            alert(`❌ NO HAY COINCIDENCIA: ${data.message}`);
            showMessage(`No hay match. ${data.message}`);
        }
    } catch (error) {
        console.error("Error de conexión con el Agente Java:", error);
        alert("❌ Error: No se pudo conectar al Agente Local de Java (http://localhost:8080). Asegúrate de que esté corriendo.");
