...
export async function GET() {
  try {
    ...
    const [rows] = await connection.execute(`
      ...
      GROUP BY b.id, b.nombre_bloque, b.tipobloque
      ORDER BY total_fraternos DESC
    `);
    ...
    // Título del reporte
    page.drawText('REPORTE: FRATERNOS POR BLOQUE', {
      x: margin,
      y: height - 170,
      size: 14,
      font: boldFont,
    ...
    page.drawText('TIPO', { x: margin + 120, y: yPosition - 18, size: 9, font: boldFont, color: darkColor });
    page.drawText('TOTAL', { x: margin + 180, y: yPosition - 18, size: 9, font: boldFont, color: darkColor });
    page.drawText('ACTIVOS', { x: margin + 230, y: yPosition - 18, size: 9, font: boldFont, color: darkColor });
    page.drawText('VARONES', { x: margin + 290, y: yPosition - 18, size: 9, font: boldFont, color: darkColor });
    page.drawText('MUJERES', { x: margin + 350, y: yPosition - 18, size: 9, font: boldFont, color: darkColor });
    page.drawText('% ACTIVOS', { x: margin + 410, y: yPosition - 18, size: 9, font: boldFont, color: darkColor });
    ...
    (rows as any[]).forEach((bloque, index) => {
      ...
      const porcentajeActivos = bloque.total_fraternos > 0 ? ((bloque.activos / bloque.total_fraternos) * 100).toFixed(1) : '0.0';

      // Datos
      page.drawText(bloque.nombre_bloque || 'Sin nombre', { x: margin + 10, y: yPosition, size: 8, font: font, color: darkColor });
      page.drawText(bloque.tipobloque || 'No especificado', { x: margin + 120, y: yPosition, size: 8, font: font, color: darkColor });
      page.drawText(`${bloque.total_fraternos}`, { x: margin + 180, y: yPosition, size: 8, font: font, color: darkColor });
      page.drawText(`${bloque.activos}`, { x: margin + 230, y: yPosition, size: 8, font: font, color: darkColor });
      page.drawText(`${bloque.varones}`, { x: margin + 290, y: yPosition, size: 8, font: font, color: darkColor });
      page.drawText(`${bloque.mujeres}`, { x: margin + 350, y: yPosition, size: 8, font: font, color: darkColor });
      page.drawText(`${porcentajeActivos}%`, { x: margin + 410, y: yPosition, size: 8, font: font, color: darkColor });
    ...
    return new Response(pdfBytes, {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="fraternos-bloques-fmn.pdf"',
      },
    });

  } catch (error) {
    console.error('Error en reporte bloques:', error);
